...
- Cross-species/HGPS work must distinguish direct conserved measurements from triangulation across non-identical designs and must remain capable of a negative verdict.
...
`110_JM101_JM105_integrate_intronsaurus.py`, `111_JM101_STAR_align_array.sbatch`, `112_JM101_integrate_after_STAR.sbatch`, Rsubread Step 2 hard-resume/turbo, Step 3 DESeq2, IRFinder drafts, complete vNext3/vNext3AE builders/readers, JM134 analysis utilities, `141_intronsaurus_matched_rna_protein_gene_stories_v3Y.py`, `26_paired_gene_body_normalized_leakage_test.py`, `28_make_synopsis_aligned_all_intron_RNAseq_plots.py`, and the complete `JM105_HGPS_Metazoan_Conservation` script/launcher/environment package. Status: exact full source not yet recovered unless already listed canonical or preserved on the named branch.

### HGPS/metazoan conservation recovery status

A 2026-07-21 File Library transcript verifies an active distributed Euler analysis under:
...
```text
human reference: 7873443
mouse reference: 7873444
sample array:    7873466
aggregate:       7873467
sample outputs:  31/34 complete
aggregate state: CANCELLED
```

Verified small provenance paths:
...
Status: `PARTIAL / SOURCE LOCATED`. The transcript is not complete runnable code. Exact next target: recover all source text, launchers, environment files, hashes, reference builders, sample-array code, aggregate code and final successful rerun proof from the Euler project root. Do not reconstruct from transcript fragments.
...
- New HGPS/metazoan distributed-analysis evidence recorded from File Library.
- Euler project root: `/cluster/scratch/jmccarthy/JM105_HGPS_Metazoan_Conservation`.
- Jobs verified: human reference `7873443`, mouse reference `7873444`, sample array `7873466`, aggregate `7873467`.
- Captured checkpoint: 31/34 sample outputs complete; aggregate cancelled.
- Exact complete source remains `PARTIAL / SOURCE LOCATED`; transcript fragments were not imported as code.
- Next recovery target is the complete source/environment/launcher/hash tree and successful aggregate rerun proof.
- Scientific boundaries preserved: no conservation claim inferred from job completion; RNA/transcript results remain distinct from protein abundance; no fake values or missing-sample imputation introduced.
