...
def map_trap_files(traps_dir):
    ...
    for fpath in all_files:
        ...
        if pos_m and trap_m:
            file_map[(pos_m.group(1).lower(), int(trap_m.group(1)))] = fpath
    return file_map
...
def stratified_trap_split(trap_orders):
    ...
    for key in trap_orders:
        by_pos.setdefault(key[1], []).append(key)

    train_keys, cal_keys, test_keys = set(), set(), set()
    for pos, keys in sorted(by_pos.items()):
        keys = list(keys)
        rng.shuffle(keys)
        n = len(keys)
...
def build_model(num_classes):
    ...
    model = models.Model(
        inputs=inputs,
        outputs={"state": state_logits, "division": division_logit, "dead": dead_logit},
        name="aging_chip_multitask_rls",
    )
    rls_probe = models.Model(inputs, [division_logit, dead_logit], name="rls_probe")
...
def make_dataset(X, y_state, y_div, y_dead, indices, training):
    Xi = X[indices]
    ys = y_state[indices]
    yd = y_div[indices].reshape(-1, 1)
    ydead = y_dead[indices].reshape(-1, 1)
    ds = tf.data.Dataset.from_tensor_slices(
        (Xi, {"state": ys, "division": yd, "dead": ydead})
    )
    if training:
...
def predict_rls_heads(rls_probe, X, indices=None):
    ...
    div_logits, dead_logits = rls_probe.predict(ds, verbose=0)
...
def evaluate_split(div_prob_by_trap, dead_prob_by_trap, human_rls, div_cfg, dead_cfg):
    rows = []
...
def candidate_endpoint(rls_probe, X, split_idx, meta_list, trap_orders, human_rls):
    div_p, dead_p = predict_rls_heads(rls_probe, X, split_idx)
    ...
    _, metrics = evaluate_split(div_by, dead_by, human_rls, div_cfg, dead_cfg)
    return div_cfg, dead_cfg, metrics
...
class RLSEndpointCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        div_cfg, dead_cfg, metrics = candidate_endpoint(
            self.rls_probe,
            self.X,
            self.cal_idx,
            self.meta_list,
...
# EXPORT
# ==============================================================================
def export_inference_models(model, class_names, div_cfg, dead_cfg, test_metrics, split_info, deployment_ok):
    ...
    rls_model = models.Model(
        model.input,
        [model.get_layer("division").output, model.get_layer("dead").output],
...
if __name__ == "__main__":
    ...
    print(f"--> Trap directory:  {TRAPS_DIR}")
    ...
    # Trim each escaped trap at the first escape frame.
    escaped = {}
    ...
    print(f"--> TIFF trap stacks found: {len(file_map)}")
    X, y_state, meta_list = load_frames(df, file_map)
    if len(X) == 0:
        sys.exit(
            f"\nFATAL: no annotated frames matched TIFF stacks.\n"
            f"TRAPS_DIR={TRAPS_DIR}\n"
            f"Check AGING_TRAPS_DIR and transferred *_trap_*.tif files."
        )

    trap_orders = build_trap_orders(meta_list)
    y_div, y_dead, human_rls = build_endpoint_targets(meta_list, trap_orders)
    ...
    train_ds = make_dataset(X, y_state, y_div, y_dead, train_idx, training=True)
    cal_ds = make_dataset(X, y_state, y_div, y_dead, cal_idx, training=False)
    ...
    stage1_div, stage1_dead, stage1_metrics = candidate_endpoint(
    ...
    stage2_div, stage2_dead, stage2_metrics = candidate_endpoint(
        rls_probe, X, cal_idx, meta_list, trap_orders, human_rls
    )
    ...
    if endpoint_rank(stage2_metrics) < endpoint_rank(stage1_metrics):
        winner = "stage2"
        chosen_div, chosen_dead, chosen_cal_metrics = stage2_div, stage2_dead, stage2_metrics
        print("--> WINNER: Stage 2 improved the RLS endpoint; keeping fine-tuned weights.")
    else:
        winner = "stage1"
        model.load_weights(stage1_final_ckpt)
        chosen_div, chosen_dead, chosen_cal_metrics = stage1_div, stage1_dead, stage1_metrics
        print("--> WINNER: Stage 1. Fine-tuning did NOT improve RLS, so it was rejected.")

    # --------------------------- UNTOUCHED TEST -------------------------------
    test_div_p, test_dead_p = predict_rls_heads(rls_probe, X, test_idx)
    ...
    test_report, test_metrics = evaluate_split(
        test_div_by, test_dead_by, human_rls, chosen_div, chosen_dead
    )
    ...
    print(
        ...
        f"persistence={chosen_dead['persistence']}"
    )
    ...
    export_inference_models(
        model, class_names, chosen_div, chosen_dead, test_metrics, split_info, deployment_ok
    )
