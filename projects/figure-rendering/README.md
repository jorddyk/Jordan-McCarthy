...
| `panel-renderers/jm105-fig4bc-sequence-architecture/` | imported from the accepted 2026-07-09 Fig4B/C provenance + layout repair session | Polished selected-vs-background renderer for JM105 Figure 4B/C sequence-architecture panels. Includes standalone Python, Euler bash wrapper, Windows runner, README, and learning-loop notes. | Uses real JM105 selected/background source tables and sequence/annotation feature tables on Euler. Selected n=49; background n=232. U1 is shown as integer paired bases from `u1_best_WC_pairs + u1_best_GU_pairs`; percent is retained only in audit TSVs. No fake data. |
