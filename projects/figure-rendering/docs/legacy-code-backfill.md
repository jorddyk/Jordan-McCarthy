...
| 12 | `panel-renderers/jm105-figure2-v10-lane-locked/render_jm105_figure2_v10_lane_locked.py` | Google Drive document "CURRENT — JM105 Figure 2 v10 lane-locked renderer register" (`projects/figure-rendering/prompts` parent folder `06 Current Figure Rendering Code`, created 2026-07-28); register states the complete runnable source file is `JM105_figure2_render_v10_lane_locked_20260728.py`, supersedes the earlier `JM105_figure2_render_v9_FINAL_20260728.py`, records SHA-256 `fbd47169d0320e04c0eaf41692b9500be4b1cfa8f862e089b891507c497bd00e`, fixed canvas 7.2×9.6in, schema `JM105-FIG2-LANE-LOCKED-v10-20260728`, and Euler destination `/cluster/home/jmccarthy/JM105_NMD_AUDIT/` | Render JM105 Figure 2 (panels a–f) with per-object fixed inch-based lanes, fail-closed clipping/collision audit, `effective_fragments` library-size unit (Novogene raw mates ÷ 2 × Effective fraction, explicitly not mapped-read CPM), and independent gene-body log2FC in panel e | `SOURCE DESCRIBED, NOT LOCATED`: the Drive register document is a metadata/identity record, not the `.py` file body; no source bytes were found under this filename anywhere in Drive or Gmail during the 2026-08-09 pass. Do not reconstruct from the register's panel/scientific-correction description. Also note the register itself states the real-data Euler render had not yet completed/passed `render_self_audit.json` as of 2026-07-28. |

## 2026-08-09 recovery pass

Google Drive was searched for recent (post-2026-07-28) figure-rendering artifacts. The only new lead is the JM105 Figure 2 v10 lane-locked renderer register documented above (priority 12): a metadata/register Google Doc naming the exact current-canonical filename, SHA-256, and Euler destination for a v10 renderer that supersedes v9, but the connector does not expose the underlying `.py` bytes. No other new figure-rendering script bodies were found in Drive or Gmail attachments since the last pass. Existing draft PR #13 (`agent/add-jm105-figure-analysis-scripts-20260730`) already carries a hash-verified, `py_compile`-validated Figure 3 CR-selectivity diagnostic renderer and a Figure 4 Mud1-coupling reference renderer against current `main`; it remains open/draft and unmerged and is not duplicated here. No code was reconstructed from the v10 register's prose description.
