...
3. Verify source paths, table schemas, row and group counts, gene aliases, and renderer arguments before rendering.
4. Check PowerShell syntax, Bash syntax, and Python compilation before delivery.
...
7. Stop the run when clipping, text overlap, legend-data contact, point-label contact, missing objects, visible systematic identifiers, or missing artifacts are detected.
...
11. Keep the accepted canonical renderer in the repository; describe failed intermediate attempts in the incident record rather than storing many near-duplicate scripts.