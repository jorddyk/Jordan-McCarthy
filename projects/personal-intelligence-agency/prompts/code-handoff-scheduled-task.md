# Code Handoff Scheduled Task Prompt

## Recovery metadata

- Human purpose: daily scheduled prompt for maintaining the private canonical GitHub code vault `jorddyk/Jordan-McCarthy`, recovering legacy code, keeping code organized by project/purpose, updating docs/wiki/handoffs, and emailing a concise code handoff.
- Original source clue/conversation context: recovered exactly from active ChatGPT scheduled task `Code Handoff` during the 2026-07-08 legacy code backfill continuation.
- Expected inputs: GitHub repository state, old ChatGPT/project context, accessible memory, File Library/file-search results, Google Drive, Gmail attachments if relevant, and other Jordan-authorized sources.
- Expected outputs: canonical code or documented recovery clues under project-first repo paths, updated README/wiki/handoff/backfill metadata, and email to `jordymac18@gmail.com` with subject `Daily code handoff`.
- Known assumptions: repo is in initial legacy-backfill phase; exact complete code recovery is preferred over speed; Markdown-only maintenance can go directly to `main`, while substantial or ambiguous runnable code should use a branch/PR.
- Data status: prompt/spec artifact only. No biological data, no private source result dump, and not runnable code.
